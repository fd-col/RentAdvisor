...
{ 
    ...
    public function get_numero_contratti($filtri){
        $annunci_assegnati = Annuncio::join('Contratto', 'Contratto.id_annuncio','=','Annuncio.id');
        
        if(isset($filtri['data_inizio_filtro'])){
            if(!is_null($filtri['data_inizio_filtro']))
                $annunci_assegnati->where('data_inizio', '>=', $filtri['data_inizio_filtro']);
        }
        if(isset($filtri['data_fine_filtro'])){
            if(!is_null($filtri['data_fine_filtro']))
                $annunci_assegnati->where('data_inizio', '<=', $filtri['data_fine_filtro']);
        }
        if(isset($filtri['tipologia'])){
            if($filtri['tipologia']=='appartamento') 
                $annunci_assegnati->where('tipologia', $filtri['tipologia']);
            if($filtri['tipologia']=='posto_letto')
                $annunci_assegnati->where('tipologia', $filtri['tipologia']);
        }
        $output= count($annunci_assegnati->get());
        return $output;
    }
